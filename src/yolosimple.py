...
class UnetUppHead_my(nn.Cell):
    def __init__(self,out_channels=1,features=[32,64,128,256,512,1024],):
        ...
        for feature in reversed(features[:-1]):
            ...
            self.ups.append(
                nn.SequentialCell([nn.Conv2dTranspose(feature,feature//2,kernel_size=2,stride=2,),
                nn.BatchNorm2d(feature//2),
                nn.LeakyReLU(0.1)])            
            )

        self.ups[8] = DoubleConv(features[0],features[0])
...
class YOLOv3(nn.Cell):
    ...
    def construct(self, x):
        ...
        return big_object_output, medium_object_output, [], features
...
class YOLOV3DarkNet53(nn.Cell):
    ...
    def construct(self, x):
        ...
        """
        # self.unetup.set_train(True)
        # self.feature_map.set_train(False)
        # self.unetup.requires_grad=False
        ...
        # [32, 64, 128, 256, 512, 1024]
        # if ops.shape(big_object_output)!=ops.shape(small_object_output):
        #     small_object_output = ops.ResizeBilinear(ops.shape(big_object_output)[2:])(small_object_output)
        ...
        return output_big, output_me, [], seg_road
...
class YoloWithLossCell(nn.Cell):
    ...
    def construct(self, x, y_true_0, y_true_1, y_true_2, gt_0, gt_1, gt_2, mask):
        ...
        loss = 0.01*loss_l +0.01*loss_m# + loss_unet
        print(loss_l, loss_m, loss_unet, loss, '\r')
        return loss
