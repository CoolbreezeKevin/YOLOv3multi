...
class UnetUppHead(nn.Cell):
    def __init__(self,out_channels=1,features=[32,64,128,256,512,1024],):
        super(UnetUppHead,self).__init__()
        self.ups = nn.CellList()
        for feature in reversed(features):
            self.ups.append(
                nn.Conv2dTranspose(feature*2,feature,kernel_size=2,stride=2,)
            )
    ...
    def construct(self, features):
        ...
        for idx in range(0,2*len(self.features),2):
            x=self.ups[idx](x)
            skip_connection=skip_connections[idx//2]

            # if x.shape!=skip_connection.shape:
            #     x=ms.resize(x,size=skip_connection.shape[2:])

            if ops.shape(x)!=ops.shape(skip_connection):
                x = ops.ResizeNearestNeighbor(ops.shape(skip_connection)[2:])(x)
